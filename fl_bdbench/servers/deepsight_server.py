...
class DeepSightServer(AnomalyDetectionServer):
    ...
    def detect_anomalies(self, updates: List[Dict]) -> List[int]:
        """
        ...
        local_model_updates = [params for _, _, params in updates]
        ...
        for cluster in np.unique(final_clusters):
            if cluster == -1:
                ...
                benign_clients_indices.extend([i for i in indexes if labels[i] == 1])
            else:
                indexes = np.argwhere(final_clusters == cluster).flatten()
                if np.sum(labels[indexes]) / len(indexes) < self.deepsight_tau:
                    benign_clients_indices.extend(indexes)

        log(INFO, f"DeepSight: Selected {len(benign_clients_indices)} benign clients")
        malicious_clients_indices = [i for i in range(len(updates)) if i not in benign_clients_indices]
        return malicious_clients_indices

...
from fl_bdbench.models import get_lstm_model, get_transformer_model, get_albert_model
...
def get_model(model_name, num_classes, dataset_name, pretrain_model_path=None):
    ...
    if "NIST" in dataset_name:
        ...
        if model_name not in ["lstm", "albert"]:
            raise ValueError(f"Model {model_name} is not supported for {dataset_name} dataset.")
        if model_name == "albert":
            log(INFO, f"Loading pretrained Albert model: {model_name}")
        ...
        if model_name == "lstm":
            ...
            model = get_transformer_model(dataset_name.lower(), num_tokens=num_classes)
            log(INFO, f"Created Transformer model for {dataset_name}")
    else:

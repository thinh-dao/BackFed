'''
VGG implementation for Tiny-ImageNet dataset.
'''
import math
import torch
...
__all__ = [
    ...
    'vgg19_bn', 'vgg19', 'get_tinyimagenet_vgg_model'
]
...
class VGG(nn.Module):
    def __init__(self, vgg_name='VGG11', num_classes=200, batch_norm=False,
                 classifier_type='simple', dropout=0.5):
        super(VGG, self).__init__()

        if vgg_name not in cfg:
            raise ValueError(f"Unknown VGG variant: {vgg_name}")
        ...
        # === [2] Adaptive pooling for 64×64 (key change) ===
...
def get_tinyimagenet_vgg_model(vgg_name, num_classes=10, batch_norm=True, classifier_type=None):
    """
